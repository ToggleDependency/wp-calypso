...
const LanguagePicker = ( {
	...
	const { __ } = useI18n();
	const [ filter, setFilter ] = useState< string >( defaultLananguageGroupId );
	...
	const getFilteredLanguages = () => {
		switch ( filter ) {
			...
			default: {
				const languageGroup = find( languageGroups, ( l: LanguageGroup ) => l.id === filter );
				const subTerritories = languageGroup ? languageGroup.subTerritories : [];
				return languages
					.filter( ( language: Language ) =>
						language.territories.some( ( t ) => includes( subTerritories, t ) )
					)
